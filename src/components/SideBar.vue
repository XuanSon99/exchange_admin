<template>
  <section>
    <v-app-bar color="secondary" app class="px-3">
      <v-btn icon @click.stop="drawer = !drawer" v-if="$vuetify.breakpoint.width < 1025">
    ...
    <v-navigation-drawer v-model="drawer" :mini-variant.sync="mini" :mini-variant-width="64"
      :temporary="$vuetify.breakpoint.width < 1024" color="secondary" app>
      <v-list-item class="pl-3 pr-0 py-5 logo">
        <v-list-item-title class="text-capitalize d-flex align-center" align="center">
          <img src="/img/logosg.png" />
          <h2 class="px-3">Admin</h2>
        </v-list-item-title>
        <v-btn icon @click.stop="mini = !mini" v-if="$vuetify.breakpoint.width > 1024">
      ...
      <v-list nav dense rounded>
        <v-list-item-group color="primary">
          <v-list-item v-for="(item, i) in items" :key="i" :to="item.route" class="py-1 px-4">
            <v-list-item-icon>
    ...
    <v-dialog v-model="dialog" max-width="400px">
      <v-card>
        ...
        <v-divider></v-divider>
        <div class="mx-6 mt-6">
          <v-text-field v-model="password" outlined type="password" label="Password" clearable></v-text-field>
        </div>
        <v-divider></v-divider>
        <v-card-actions>
          <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    getInfo() {
      this.CallAPI(
        "get",
        "manage/info/" + localStorage.getItem("username"),
        {},
        (res) => {
